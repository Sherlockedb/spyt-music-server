...
class DownloaderService:
    ...
    async def get_task_status(self, task_id: str) -> Optional[Dict[str, Any]]:
        ...
        返回:
            任务状态信息
        """
        return await self.task_repo.find_one({"task_id": task_id})
    ...
    async def retry_failed_task(self, task_id: str) -> bool:
        ...
        return await self.task_repo.retry_task(task_id) is not None
    ...
    async def get_task_statistics(self) -> Dict[str, int]:
        ...
        # 统计各状态任务数量
        for status in [STATUS_PENDING, STATUS_IN_PROGRESS, STATUS_SUCCESS, STATUS_FAILED]:
